...
int main()
{
    ...
    while (true)
    {
        if (o == 1)
        {
            ...
            for (int i = 0; i < arrive.size(); i++)
            {
                arrival_hour[i] = (departure_hour[i] + sched2[i].flight[i].flight_cost + 8) % 24;

                cout << i + 1 << "\t\t"
                     << "| " << sched2[i].flight[i].flight_id << "\t\t|" << sched2[i].flight[i].flight_path << "\t|" << setfill('0') << setw(2) << departure_hour[i] + 8 << ":" << setfill('0') << setw(2) << departure_mins[i] << "\t\t\t| " << setfill('0') << setw(2) << arrival_hour[i] << ":" << setfill('0') << setw(2) << arrival_mins[i] << "\t\t|" << sched2[i].aircraft[i].aircraft_id << endl;
            }
